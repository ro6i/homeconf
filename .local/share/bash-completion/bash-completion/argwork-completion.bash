...
__argwork_one() {
  ...
  case "$spec" in
    ...
    here)
      local arg_list=(${@:1})
      __argwork_lookup_types["$index"]='here'
...
__argwork_complete() {
  ...
  case "$sector" in
    POSITIONAL)
      ...
      ;;
    OPTIONAL)
      if [[ "${COMP_WORDS[$(($COMP_CWORD - 1))]:0:2}" == '--' ]]
      then
        key="${COMP_WORDS[$(($COMP_CWORD - 1))]:2}"
        key_shift=0
        if [[ -z "$key" || ! -v __argwork_optional_arg_vars["$key"] ]]
        then
          return
        fi
      else
        COMPREPLY=($(compgen -P '--' -W "$(printf "%s\n" "${__argwork_optional_arg_vars[@]}")" -- "${COMP_WORDS[$COMP_CWORD]:2}"))
        return
      fi
      ;;
  esac
  ...
  case "${__argwork_lookup_types["$key"]}" in
    ...
    here)
      ...
      IFS=$'\n' COMPREPLY=($(compgen "${prefix[@]}" -W "$(printf "%s\n" "${opts_split[@]}" | grep -vwE "$visited_pattern")" -- "$comp_word"))
      ;;

    shell)
      local shell_code="${__argwork_lookup_values["$key"]}"
      IFS=$'\n' local opts_split=($(eval "$shell_code"))
      IFS=$'\n' COMPREPLY=($(compgen "${prefix[@]}" -W "$(printf "%s\n" "${opts_split[@]}" | grep -vwE "$visited_pattern")" -- "$comp_word"))
      ;;
    ...
    cmd)
      ...
      IFS=$'\n' COMPREPLY=($(compgen "${prefix[@]}" -W "$(printf "%s\n" "$(eval "$command_line")" | grep -vwE "$visited_pattern")" -- "$comp_word"))
      ;;
...
_argwork_completion() {
  ...
  local argwork_script_path="${COMP_WORDS[1]}"
  ...
  if [[ "${COMP_WORDS[$COMP_CWORD]}" == '?' && ($COMP_TYPE -eq 63 || $COMP_TYPE -eq 9) ]]
  then
    if [[ $COMP_TYPE -eq 9 ]]
    then
      return
  ...
  elif [[ "${COMP_WORDS[$COMP_CWORD]}" == '#' && ($COMP_TYPE -eq 63 || $COMP_TYPE -eq 9) ]]
  then
    if [[ $COMP_TYPE -eq 9 ]]
    then
      return
  ...
  then
    ...
    local script_sub_rel_dir="$script_sub_rel"
